...
def run_backup(
    ...
    try:
        ...
        logger.info("Changes: %d new/modified, %d deleted, %d unchanged",
                     len(changed), len(deleted), len(unchanged))
        ...
        try:
            ...
            if unchanged and old_manifest.backup_time:
                logger.info("Copying %d unchanged files (parallel) from previous snapshot...",
                           len(unchanged))
                copy_unchanged_files(
        ...
        save_manifest(new_manifest, manifest_path)
        logger.info("Backup complete: backups/%s/ (%d new/modified, %d deleted)",
                    now_str, len(changed), len(deleted))
    except Exception:
